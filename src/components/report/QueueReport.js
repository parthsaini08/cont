...
const QueueReport = () => {
  ...
  const fetchPeriod = async (period) => {
    ...
    const res = await fetch(
      `${BASE_URL}get_summary.php?${params.toString()}`,
      { credentials: "include" }
    );
  ...
  const fetchReport = async () => {
    ...
    const p2Index = {};
    p2.forEach((r) => {
      p2Index[r.queue_name] = r;
    });

    const merged = p1.map((r1) => {
      const r2 = p2Index[r1.queue_name];

      return {
        queue_name: r1.queue_name,
        period1: {
  ...
  const downloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Queue Comparison");

    const buffer = XLSX.write(wb, { bookType: "xlsx", type: "array" });
    saveAs(new Blob([buffer]), "queue-comparison.xlsx");
  };
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.map((r, i) => (
                ...
                >
                  <td className="p-3">{r.queue_name}</td>
